...
public class MainActivity extends ActionBarActivity {
    ...
    private void fetch() {
        ...
        JsonObjectRequest request = new JsonObjectRequest(
                "https://drive.google.com/uc?export=download&id=0B2EzV-yWklmfX1k1Tk9yb2tTaGc" ,//jsonデータが有るサーバーのURLを指定します。
                null,
    ...
    private List<MessageRecord> parse(JSONObject json) throws JSONException {
        ...
        for(int i=0; i < jsonMessages.length(); i++) {
            ...
            int noi = i + 1;
            String number = jsonMessage.getString("number") + " No." + String.valueOf(noi);
            //jsonMessageを新しく作ります。
            MessageRecord record = new MessageRecord(url, title, number);
